...
ggplot(subset(ABIS_F_long,CellType =="B_mem"|CellType =="B_n"|CellType =="Plasmablasts"), aes(fill=Infected,x=Infected, y=Absolute)) +
  geom_boxplot(position="dodge", outlier.size = 1)+
  scale_fill_manual(breaks = c("Neg","Pos"),
                     values=c( "#7788e1", "#f38796")) +
  theme_classic() +
    facet_wrap(~CellType, scales="free_y")
ggsave("Female_posneg_Bcell_ABIS.pdf", width=6, heigh=2)  
```
...
```{r}
#Infection Females
cyber_F_wide= subset(cyber_absolute_wide, Sex=="F")
cyber_F_long= subset(cyber_absolute_long, Sex=="F")

cyber_F_Ifxn_test = subset(cyber_F_long, select=-c(Sex,Category))
cyber_F_Ifxn_test =  cyber_F_Ifxn_test%>%
  pivot_wider(
    names_from = c(CellType, Infected),
    values_from = c(Absolute))


test_F_Ifxn_abs=data.frame()

for (cell in colnames(cyber_F_wide)[5:26]) {
  temp=wilcox.test(cyber_F_Ifxn_test[[paste(cell,"Neg",sep="_")]], cyber_F_Ifxn_test[[paste(cell,"Pos",sep="_")]],  alternative="two.sided", data=cyber_F_Ifxn_test, exact=FALSE)
  test_F_Ifxn_abs=rbind(test_F_Ifxn_abs,c(paste(cell),c("PosvNeg_F"),temp$p.value))
}
  
colnames(test_F_Ifxn_abs)=c("CellType","Comparison", "Wilcox_P")
test_F_Ifxn_abs$FDR=p.adjust(test_F_Ifxn_abs$Wilcox_P, method = "BH", n = length(test_F_Ifxn_abs$Wilcox_P))
test_F_Ifxn_abs[order(test_F_Ifxn_abs$Wilcox_P),]
```
plots
```{r}
ggplot(subset(cyber_F_long,CellType =="B_mem"|CellType =="B_n"|CellType =="PC"), aes(fill=Infected,x=Infected, y=Absolute)) +
  geom_boxplot(position="dodge", outlier.size = 1)+
  scale_fill_manual(breaks = c("Neg","Pos"),
                     values=c( "#7788e1", "#f38796")) +
  theme_classic() +
...
ggsave("Female_posneg_Bcell_CIBERSORT.pdf", width=6, heigh=2) 
```
...
```{r}
txcell_long = txcell %>%
  pivot_longer(!c(NovogeneCode,Cat,Infection,Sex), names_to = "CellType", values_to = "Absolute")

xcell_f_posneg_wide= subset(txcell, Sex=="F")
xcell_f_posneg_long= subset(txcell_long, Sex=="F")


test_xcell_posneg_f=compare_means(Absolute ~ Infection, data = xcell_f_posneg_long, 
              group.by = "CellType", p.adjust.method="BH")

test_xcell_posneg_f=test_xcell_posneg_f[order(test_xcell_posneg_f$p.format),]
test_xcell_posneg_f
```

```{r}
ggplot(subset(xcell_f_posneg_long,CellType =="B cell memory"|CellType =="B cell"|CellType =="B cell plasma"|CellType =="B cell naive"), aes(fill=Infection,x=Infection, y=Absolute)) +
  geom_boxplot(position="dodge",width=0.7, outlier.size = 1)+ 
  scale_fill_manual(breaks = c("Neg","Pos"),
                     values=c( "#7788e1", "#f38796")) +
  theme_classic()+
...
ggsave("Female_posneg_Bcell_xcell.pdf", width=6, heigh=4)  
