...
```{r}
dds_Var=DESeqDataSetFromMatrix(countData = Counts_ECG, colData =CHAVA_ECG, design= ~Sex + PCA + Origin + Cat+ AgeCategory +  Infection)
...
pheatmap(mat_AB_Pos, annotation_col = sampleinfo_AB_Pos, show_colnames = FALSE, labels_row = topgenesnames_AB_Pos, annotation_colors= phenoCols,color = colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdBu")))(100))
dev.off()
```
...
### DEG
```{r}
ddsAB_Neg=DESeqDataSetFromMatrix(countData = Counts_ECG_AB_Neg, colData =CHAVA_ECG_AB_Neg, design= ~PCA +Sex  + Cat)
...
topgenes_AB_Neg=subset(annot_AB_Neg, log2FC < -0.58 | log2FC >0.58)
topgenes_AB_Neg=arrange(topgenes_AB_Neg, FDR)
...
pdf(file="Neg_BvA_heatmap_0.1_log2FC_0.58.pdf")
pheatmap(mat_AB_Neg, annotation_col = sampleinfo_AB_Neg, show_colnames = TRUE, labels_row = topgenesnames_AB_Neg, annotation_colors = phenoCols, color = colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdBu")))(100))
dev.off()
```
